...
- **Изоляция кейсов**: данные разных кейсов не смешиваются (**1 образ = 1 кейс**).
  - предпочтительно: отдельная БД Neo4j на кейс (`case_<image_file_name>`),
  - если используется бесплатная редакция Neo4j без multiple databases: допускается **отдельный контейнер/инстанс Neo4j на кейс** (1 контейнер = 1 БД `neo4j`).
...
- `IP.id = ip` (достаточно, так как **кейс изолирован** в своей БД/контейнере)
- `SSHLoginEvent.id = run_id + ":" + sha256(raw_line)` (или ts+pid+ip+user)

## 9) Именование кейса/БД

Система сама выбирает “имя кейса” по имени файла образа:

- `case_name = "case_" + sanitize(image_file_name)`
- `sanitize`: заменить все символы, кроме `[a-zA-Z0-9_]`, на `_`, привести к нижнему регистру, обрезать до разумной длины (например, 48–64 символа).

Это имя используется:

- как имя БД Neo4j (если поддерживаются multiple databases),
- или как имя контейнера/проекта (если кейс = отдельный контейнер Neo4j).
